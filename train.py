...
logging.info(f'Experiment name: {_c(args.expname, attr="blue")}, Dataset: {_c(args.dataset, attr="blue")}, Negative Sampling: {_c(args.neg_type, attr="blue")}')

# Data loading
data_manager = DATA_MANAGERS[args.data_manager](args)
trn_loader, val_loader, _ = data_manager.build_data_loaders()

# Experiment Name for tracking in Wandb
expname = f'{args.expname}_{args.neg_type}'

if IS_MAIN_PROC:
    wandb.init(project=f'{args.project}_{args.dataset}', name=expname)
...
# Training loop
scaler = torch.GradScaler(device=args.device)
global_step = 0
...
for epoch in range(args.num_epochs):
    ...
    if args.neg_type in ('shorty', 'mns', 'bmns', 'cbns', 'cbmns', 'mbns', 'in-batch'):
        trn_loader.collate_fn.neg_type = args.neg_type if hasattr(args, 'hard_neg_start') and epoch > args.hard_neg_start else 'in-batch'
    ...
    for i, b in enumerate(t):
        ...
        global_step += 1
        t.set_description('Epoch: %d/%d, Loss: %.4E'%(epoch, (args.num_epochs-1), (epoch_loss/(i+1))), refresh=True)
