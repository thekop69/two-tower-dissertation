...
class TFEncoder(BaseNet):
    ...
    def encode(self, b):
        with torch.autocast(device_type=self.get_device().type, enabled=torch.amp.autocast_mode.is_autocast_available(device_type=self.get_device().type)):
            embs = b['xfts']
...
class DistClassifierNetwork(DistBaseNetwork):
    ...
    def forward_backward(self, b, loss_fn, scaler=None):
        # forward
        with torch.autocast(device_type=self.get_device().type, enabled=scaler is not None):
...
class DistDualEncoderHNM(DistDualEncoderAll):
    ...
    def update_non_parameters(self, epoch, step, *args, **kwargs):
        ...
        if self.update_trn_shorty and epoch_end:
            ...
            trn_loader.collate_fn.neg_type = self.neg_type if self.neg_type in ('shorty', 'mns', 'bmns', 'cbns', 'cbmns', 'mbns') else 'in-batch'
